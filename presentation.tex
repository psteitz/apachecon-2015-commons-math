...
\begin{itemize}
  ...
  \item Basic combinatorics (\(\binom{n}{k}, n!\), Stirling numbers...)
  \item Basic integer arithmetic (checked ops, gcd, lcm, primality...)
...
\begin{small}
\begin{itemize}
  \item Probability distributions (Gaussian, Exponential, Poisson...)
  \item Random number generators (Well, ISAAC, Mersenne twister...)
  \item Random data generators (samplers, random vectors...)
  \item Univariate statistics (storeless and in-memory)
  \item Regression (matrix-based and storeless)
  \item Correlation
...
\begin{small}  
 {\fblue{First problem:  escape to NaN}}
\begin{itemize}
...
distribution functions of the two samples. \\
 \begin{itemize}
  ...
  \item What to do for mid-size samples (\( 100 < m \times n < 10,000\))?
...
\begin{itemize}
\item LevenbergMarquardtOptimizer
\item GaussNewtonOptimizer 
\end{itemize}
\end{frame}
...
\begin{small}
\begin{itemize}
\item As of 3.4.1, by default ties are averaged and NaNs cause NotANumberException
...
\end{itemize}
\item Default in Spearman's is natural ranking on doubles with ties averaged
and NaNs disallowed
\end{itemize}
\end{small}
\end{frame}
...
NaNStrategies:
\begin{itemize}
\item MINIMAL - NaNs are treated as minimal in the ordering
\item MAXIMAL - NaNs are treated as maximal in the ordering
\item REMOVED - NaNs are removed
\item FIXED - NaNs are left "in place" 
\item FAILED - NaNs cause NotANumberException
\end{itemize}
Problems: 
\begin{enumerate}
...
\begin{small}
\begin{itemize}
  \item Check out the user guide \url{http://commons.apache.org/math/userguide}
  \item Ask - and answer - questions on the user mailing list
  \item Participate in discussions on the dev list
  \item Create bug reports / feature requests / improvements in JIRA
  \item Create and submit patches (code, documentation, examples)
  \item Look at \url{http://commons.apache.org/math/developers.html}
  \item Run \mintinline{latex}{mvn site} and review CheckStyle, Findbugs reports
   when creating patches
  \item Don't be shy asking for help with maven, git, coding style, math ... anything
